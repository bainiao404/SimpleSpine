...
2. **注册全局实例**：使用 `const PIXI = registerPIXI(PIXI_ORIG)` 接口将 `@pixi/node` 实例注册给库。由于 Node 中直接导入的 Module 命名空间是只读不可修改的，`registerPIXI` 在内部会自动检测并为其创建浅拷贝的可写代理对象，挂载 Spine 运行时的同时将该可写实例返回，方便后续直接使用。
